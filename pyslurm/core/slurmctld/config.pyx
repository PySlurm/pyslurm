...
cdef class Config:
    ...
    def accounting_store_flags(self):
        ...
        if flags & slurm.CTL_CONF_SJC:
            out.append("JOB_COMMENT")
        if flags & slurm.CTL_CONF_SJE:
            out.append("JOB_ENV")
        if flags & slurm.CTL_CONF_SJX:
            out.append("JOB_EXTRA")
        if flags & slurm.CTL_CONF_SJS:
    ...
    def allow_spec_resource_usage(self):
        if self.ptr.conf_flags & slurm.CTL_CONF_ASRU:
            return True
    ...
    def disable_root_jobs(self):
        if self.ptr.conf_flags & slurm.CTL_CONF_DRJ:
            return True
        return False
    ...
    def track_wckey(self):
        if self.ptr.conf_flags & slurm.CTL_CONF_WCKEY:
            return True
        return False

    @property
    def use_pam(self):
        if self.ptr.conf_flags & slurm.CTL_CONF_PAM:
            return True
        return False
