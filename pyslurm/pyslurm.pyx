...
cdef inline SLURM_VERSION_NUMBER(): return 0x020304
	...
	cpdef dict __get(self):
		...
		if self._Partition_ptr is not NULL:
			...
			for i from 0 <= i < self._Partition_ptr.record_count:
				...
				Part_dict[u'nodes'] = self._record.nodes
...
cdef class node:
	...
	cpdef __get(self):
		...
		for i from 0 <= i < self._Node_ptr.record_count:
			...
			Host_dict['gres'] = slurm.gresToDict(self._Node_ptr.node_array[i].gres)
