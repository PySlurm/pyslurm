...
#    assert isinstance(test_partition, basestring)
    ...
    assert_equals(obj.priority_job_factor, sctl["PriorityJobFactor"])
    assert_equals(obj.priority_tier, sctl["PriorityTier"])
    assert_equals(obj.qos, sctl["QoS"])
    assert_equals(obj.req_resv, sctl["ReqResv"])
    assert_equals(obj.root_only, sctl["RootOnly"])
    assert_equals(obj.select_type_parameters, sctl.get("SelectTypeParameters").split(","))
    assert_equals(obj.over_subscribe, sctl["OverSubscribe"])
